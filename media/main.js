...
(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const pendingOperationsContainer = document.querySelector(
      ".pending-operations"
    );
      ...
      if (!containsFileContent && currentAttachments.length > 0) {
        ...
        attachmentsToSend = currentAttachments.map((attachment) => {
          ...
          if (attachment.content) {
            log(
              `Including content for attached file: ${
                attachment.name || attachment.path.split(/[\\\/]/).pop()
              }, length: ${attachment.content.length} characters`
            );
          } else {
            log(
        ...
        log(
          "Message already contains file content, skipping attachments to avoid duplication"
        );
      }
  ...
  function updateAttachmentsUI() {
    ...
    currentAttachments.forEach((attachment, index) => {
      ...
      icon.classList.add(
        attachment.type === "file" ? "codicon-file" : "codicon-folder"
      );
  ...
  function updatePendingOperationsUI() {
    ...
    console.log(
      "Updating pending operations UI with",
    ...
    pendingOperations.forEach((operation) => {
      ...
      title.innerHTML = `
                ...
                <div class="operation-file">
                    <span class="file-name">${getFilename(
                      operation.filePath
                    )}</span>
      ...
      if (operation.type === "update") {
        ...
        viewDiffButton.addEventListener("click", () => {
          ...
          // Toggle diff view
          const diffContainer =
            operationElement.querySelector(".diff-container");
          if (diffContainer) {
            diffContainer.style.display =
              diffContainer.style.display === "none" ? "block" : "none";
            viewDiffButton.innerHTML =
      ...
      switch (message.command) {
        ...
        case "updatePendingOperations":
          if (Array.isArray(message.operations)) {
            ...
            console.warn(
              "Received invalid operations data:",
              message.operations
            );
          }
          break;
        ...
        case "operationDiff":
          if (message.id && message.diff) {
            const operationElement = document.querySelector(
              `.operation-item[data-id="${message.id}"]`
            );
            if (operationElement) {
              const diffContainer =
                operationElement.querySelector(".diff-container");
        ...
        case "fileContentLoaded":
          if (message.path && message.content) {
            ...
            log(
              `File content loaded for ${message.path}, length: ${contentLength} characters`
            );

            // Update the attachment with the content
            const attachment = currentAttachments.find(
              (a) => a.path === message.path
            );
            if (attachment) {
              ...
              const fileName =
                attachment.name || attachment.path.split(/[\\\/]/).pop();
              log(
                `Updated attachment with content for ${fileName}, content length: ${contentLength}`
              );
            } else {
              log(
                `Warning: Received content for ${message.path} but no matching attachment found`
              );
              ...
              updateAttachmentsUI();
              log(
                `Added new attachment for ${fileName} with content length: ${contentLength}`
              );
            }
          }
          break;
  ...
  function showFileSuggestions(searchTerm) {
        ...
        workspaceFolders.filter(
          (folder) =>
            ...
            (folder.name.toLowerCase().includes(searchTerm) ||
              (folder.parent &&
                folder.parent.toLowerCase().includes(searchTerm)))
        )
      );
    ...
    const createSuggestionGroup = (parentPath, items) => {
      ...
      if (
        parentPath !== "/" &&
        items.some((item) => item.level && item.level > 1)
      ) {
    ...
    const prioritizedGroupKeys = Object.keys(itemsByPath).sort((a, b) => {
      const aHasExactMatch = itemsByPath[a].some(
        (item) => item.name.toLowerCase() === searchTerm.toLowerCase()
      );
      const bHasExactMatch = itemsByPath[b].some(
        (item) => item.name.toLowerCase() === searchTerm.toLowerCase()
      );
    ...
    if (allItems.length > 10) {
      ...
      moreItemsIndicator.textContent = `...and ${
        allItems.length - 10
      } more items`;
  ...
  function navigateSuggestions(direction) {
    ...
    const items = Array.from(
      suggestionsContainer.querySelectorAll(
        '.suggestion-item:not([style*="display: none"])'
      )
    );
