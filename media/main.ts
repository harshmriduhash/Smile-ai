...
const addModelButtonElement = document.getElementById(
  "addModel"
) as HTMLButtonElement;
const includeImportsElement = document.getElementById(
  "includeImports"
) as HTMLInputElement;
const includeTipsElement = document.getElementById(
  "includeTips"
) as HTMLInputElement;
const includeTestsElement = document.getElementById(
  "includeTests"
) as HTMLInputElement;
const messageTemplateElement = document.getElementById(
  "message-template"
) as HTMLTemplateElement;
const codeBlockTemplateElement = document.getElementById(
  "code-block-template"
) as HTMLTemplateElement;
const fileAttachmentTemplateElement = document.getElementById(
  "file-attachment-template"
) as HTMLTemplateElement;
const attachFileButtonElement = document.getElementById(
  "attachFile"
) as HTMLButtonElement;
const attachFolderButtonElement = document.getElementById(
  "attachFolder"
) as HTMLButtonElement;
const chatModeSelectElement = document.getElementById(
  "chatMode"
) as HTMLSelectElement;
const openChatButtonElement = document.getElementById(
  "openChat"
) as HTMLButtonElement;
const openComposerButtonElement = document.getElementById(
  "openComposer"
) as HTMLButtonElement;
  ...
  currentAttachments.forEach((attachment) => {
    ...
    element.innerHTML = `
            <i class="codicon codicon-${
              attachment.type === "file" ? "file-code" : "folder"
            }"></i>
            <span>${
...
function addMessage(message: ChatMessage) {
  ...
  const messageElement = messageTemplateElement.content.cloneNode(
    true
  ) as DocumentFragment;
  ...
  avatar.classList.add(
    message.role === "user" ? "codicon-account" : "codicon-hubot"
  );
  ...
  if (
    ...
    fileAttachmentTemplateElement
  ) {
    const attachmentsContainer = document.createElement("div");
    attachmentsContainer.className = "attachments";

    message.attachments.forEach((attachment) => {
      const attachmentElement = fileAttachmentTemplateElement.content.cloneNode(
        true
      ) as DocumentFragment;
      const filename = attachmentElement.querySelector(
        ".filename"
      ) as HTMLElement;
      ...
      if (filename && icon) {
        ...
        icon.classList.add(
          attachment.type === "file" ? "codicon-file-code" : "codicon-folder"
        );
      }
  ...
  const codeBlocks = messagesContainer.querySelectorAll(
    ...
    button.addEventListener("click", (e) => {
      ...
      if (codeContent) {
        navigator.clipboard.writeText(codeContent).then(() => {
          ...
          const copyButton = (e.target as HTMLElement).closest(
            ".copy-button"
          ) as HTMLElement;
          if (copyButton) {
...
function formatMessage(content: string): string {
  ...
  return content
    .replace(/```(\w*)\n([\s\S]*?)```/g, (_, lang, code) => {
      ...
      const codeBlock = codeBlockTemplateElement.content.cloneNode(
        true
      ) as DocumentFragment;
...
function updatePendingOperationsUI() {
  ...
  pendingOperations.forEach((operation) => {
            ...
            <div class="operation-content">
                ...
                ${
                  operation.type === "update"
                    ? `<div class="operation-diff"></div>`
                    : ""
                }
                ${
                  operation.type === "add"
                    ? `<div class="operation-preview"></div>`
                    : ""
                }
            </div>
        `;
    ...
    if (operation.type === "add" && operation.newContent) {
      ...
      if (previewElement) {
        previewElement.innerHTML = `<pre class="code-preview">${escapeHtml(
...
function updateOperationDiff(
  operationId: string,
  diff: { added: boolean; removed: boolean; value: string }[]
) {
  const operationElement = document.querySelector(
    `.operation-item[data-id="${operationId}"]`
  );
