...
export class AIAssistantPanel {
  ...
  constructor(
    ...
    this.initializeAIEngine().catch((error: Error) =>
      this.handleError("AI Engine initialization failed", error)
    );

    // Setup webview asynchronously
    this.setupWebview().catch((error: Error) =>
      this.handleError("Webview setup failed", error)
    );
    ...
    this.indexCodebase().catch((error: Error) =>
      this.handleError("Codebase indexing failed", error)
    );
  }
  ...
  private showModelConfigurationRequired(): void {
    ...
    vscode.window
      .showWarningMessage(message, configureButton)
      .then((selection) => {
        if (selection === configureButton) {
          this.modelManager
            .promptAddModel()
            .catch((error) =>
              this.handleError("Model configuration failed", error)
            );
        }
      });
  }
      ...
      for (const folder of workspaceFolders) {
        ...
        for (const uri of fileUris) {
          try {
            ...
            const parent =
              segments.length > 0 ? segments.join("/") : folder.name;
  ...
  private async handleUserMessage(text: string, options: any): Promise<void> {
    ...
    try {
      console.log(
        "Handling user message with options:",
        JSON.stringify({
          hasAttachments:
            options && options.attachments ? options.attachments.length : 0,
      ...
      if (
        ...
        for (const attachment of options.attachments) {
          ...
          if (attachment.type === "file") {
            ...
            if (!attachment.content) {
              try {
                console.log(
                  `Loading content for attached file: ${attachment.path}`
                );
                const uri = vscode.Uri.file(attachment.path);
                const fileData = await vscode.workspace.fs.readFile(uri);
                attachment.content = new TextDecoder().decode(fileData);
                console.log(
                  `Successfully loaded content for ${attachment.path}, length: ${attachment.content.length} characters`
                );
              } catch (err) {
                console.error(
                  `Error reading attached file ${attachment.path}:`,
                  err
                );
              }
            }
        ...
        console.log(
          "Message already contains file content, clearing attachments to avoid duplication"
        );
        options.attachments = [];
      }
  ...
  private async handleDocumentChange(event: vscode.TextDocumentChangeEvent) {
    if (event.document === vscode.window.activeTextEditor?.document) {
      const fileContext = await this.fileAnalyzer.analyzeFile(
        event.document.uri
      );
      this.webviewView.webview.postMessage({
        type: "contextUpdate",
    ...
    try {
      ...
      if (result && result.length > 0) {
        ...
        try {
          ...
          WebviewManager.getInstance().sendFileAttached(
            "smile-ai.assistant",
            filePath,
            fileContent
          );
  ...
  private async handleAttachFolder() {
    try {
      ...
      if (result && result.length > 0) {
        ...
        WebviewManager.getInstance().sendFolderAttached(
          "smile-ai.assistant",
          folderPath
        );
  ...
  private async setupWebview(): Promise<void> {
    ...
    try {
      ...
      const welcomeMessage: Message = {
        role: "system",
        content:
          "Welcome to Smile AI! Ask me anything about your code or suggest improvements.",
        timestamp: Date.now(),
      };
      ...
      async (message) => {
        ...
        switch (message.command) {
          ...
          case "acceptPartialChange":
            await this.handleAcceptPartialChange(
              message.id,
              message.lineIndices
            );
            break;
    ...
    try {
      ...
      if (success) {
        vscode.window.showInformationMessage(
          "File operation applied successfully."
        );
      } else {
  ...
  private async handleRejectAllOperations(): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage(
        `Error rejecting all operations: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  ...
  private async handleAcceptPartialChange(
    id: string,
    lineIndices: number[]
  ): Promise<void> {
    try {
      ...
      if (typeof fileOperationManager.acceptPartialChange === "function") {
        const success = await fileOperationManager.acceptPartialChange(
          id,
          lineIndices
        );
        ...
        if (success) {
          vscode.window.showInformationMessage(
            "Partial changes applied successfully."
          );
        } else {
        ...
        vscode.window.showErrorMessage(
          "Partial change functionality not available."
        );
      }
    } catch (error) {
      console.error("Error accepting partial change:", error);
      vscode.window.showErrorMessage(
        `Error applying partial changes: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  ...
  public addMessage(message: AIMessage): void {
    ...
    WebviewManager.getInstance().sendMessage("assistant", {
      command: "addMessage",
      message,
    });
  }
