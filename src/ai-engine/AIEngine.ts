...
export class AIEngine {
  ...
  public initRAG(codebaseIndex: CodebaseIndex): void {
    ...
    console.log(
      `Initialized RAG service with ${
        this.codebaseIndex ? "codebase index" : "no index"
      }`
    );
  }
  ...
  public async testConnection(): Promise<boolean> {
    try {
      const response = await fetch(
        this.config.provider.apiEndpoint + "/api/health",
        {
          method: "GET",
        }
      );
  ...
  public async processAgentMessage(
    ...
    try {
      ...
      return `I encountered an error while trying to process your request in agent mode: ${
        error instanceof Error ? error.message : String(error)
      }`;
    }
  }

  public async processAskMessage(
    message: string,
    options: ProcessOptions
  ): Promise<string> {
  ...
  private async sendRequest(
    ...
    try {
      console.log(
        `Sending ${mode} request to ${this.config.provider.name} at ${this.config.provider.apiEndpoint}`
      );
      ...
      if (hasAttachments && !containsFileContent) {
        const attachmentsWithContent =
          options?.options?.attachments?.filter(
            (a: any) => a.type === "file" && a.content
          ) || [];

        if (attachmentsWithContent.length > 0) {
          console.log(
            `Enhancing message with ${attachmentsWithContent.length} file attachments`
          );
          enhancedMessage += "\n\n";
      ...
      const isCodebaseQuery =
        message.toLowerCase().includes("codebase") ||
        message.startsWith("@") ||
      ...
      if (isCodebaseQuery) {
        if (this.ragService && this.ragService.isEnabled()) {
          ...
          const enhancedContext = await this.ragService.enhanceQueryWithContext(
            message
          );
          ...
          codebaseContext = this.prepareCodebaseContext(
            message,
            options.codebaseIndex
          );
        }
      }
      ...
      if (enhancedMessage !== message) {
        console.log(
          "Message already enhanced with attachments, clearing attachments in request body"
        );
      ...
      const requestBody =
        this.config.provider.name === "ollama"
          ? {
      ...
      return `I received your message: "${message.substring(0, 50)}${
        message.length > 50 ? "..." : ""
      }"\n\nHowever, there was an error processing your request. ${
  ...
  private getSystemPrompt(mode: "chat" | "agent" | "ask"): string {
    ...
    const isTranslationMode =
      mode === "chat" && this.detectTranslationRequest();
  ...
  private detectTranslationRequest(): boolean {
    ...
    if (this.conversationHistory.length > 0) {
      ...
      for (const message of recentMessages) {
        if (
          message.role === "user" &&
          this.isTranslationRequest(message.content)
        ) {
          return true;
        }
      }
  ...
  private async callAIProvider(messages: AIMessage[]): Promise<AIResponse> {
    ...
    try {
      ...
      if (error.response) {
        ...
        userMessage = `Error from ${providerName} (Status ${status}): ${
  ...
  public async generateEmbeddings(text: string): Promise<number[]> {
    ...
    try {
      ...
      const embedding = new Array(embeddingLength)
        .fill(0)
        .map(() => Math.random() * 2 - 1);
  ...
  public async generateEmbedding(text: string): Promise<number[]> {
    ...
    try {
      const response = await axios.post(
        `${provider.apiEndpoint}/embeddings`,
        {
  ...
  private prepareCodebaseContext(
    _query: string,
    codebaseIndex: CodebaseIndex
  ): string {
    if (!codebaseIndex) {
    ...
    try {
      ...
      for (const regex of fileBlockRegexes) {
        ...
        while ((match = regex.exec(response)) !== null) {
          ...
          console.log(
            `- Content length: ${fileContent?.length || 0} characters`
          );
          ...
          if (fileExists) {
            ...
            if (originalContent !== fileContent) {
              const description = `Update file ${path.basename(
                filePath
              )} with AI-generated content`;
              const operation =
                await fileOperationManager.createUpdateOperation(
                  absolutePath,
                  originalContent,
                  fileContent,
                  description
              ...
              console.log(
                `File content unchanged, no update needed for ${filePath}`
              );
            }
          } else {
            // Create new file
            const description = `Create new file ${path.basename(
              filePath
            )} with AI-generated content`;
            const operation = await fileOperationManager.createAddOperation(
              absolutePath,
              fileContent,
              description
            );
  ...
  private isTranslationRequest(message: string): boolean {
    ...
    const isTranslation = translationPatterns.some((pattern) =>
      normalizedMessage.includes(pattern)
    );

    console.log(
      `Checking if message is a translation request: ${isTranslation}`,
      isTranslation
        ? `Matched pattern in: ${normalizedMessage.substring(0, 50)}...`
        : ""
    );
