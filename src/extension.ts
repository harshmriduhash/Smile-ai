...
export class SmileAIExtension {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    if (!activeModel) {
      ...
      const aiConfig: AIEngineConfig = {
        ...
        embeddingModelName:
          activeModel.embeddingModelName || "nomic-embed-text",
        enableRAG:
          activeModel.enableRAG !== undefined
            ? activeModel.enableRAG
            : enableRAG,
      };
      this.aiEngine = new AIEngine(aiConfig);
    }
    ...
    if (this.aiEngine) {
      const ragService = RAGService.getInstance(
        this.aiEngine,
        this.codebaseIndexer.getIndex()
      );
      if (ragService) {
        ragService.setEnabled(enableRAG);
        ragService.setMaxChunks(config.get<number>("rag.maxChunks", 5));
        ragService.setMaxChunkSize(
          config.get<number>("rag.maxChunkSize", 2000)
        );
        ragService.setMinSimilarity(
          config.get<number>("rag.minSimilarity", 0.7)
        );
      }
    }

    // Initialize tree view provider
    this.improvementProvider = new ImprovementTreeProvider(
      this.improvementManager
    );
    vscode.window.registerTreeDataProvider(
      "smile-ai.futureImprovements",
      this.improvementProvider
    );
  ...
  private async initializeComponents(): Promise<void> {
    try {
      ...
      vscode.workspace.onDidChangeConfiguration((e) => {
        if (
          e.affectsConfiguration("smile-ai.enableRAG") ||
  ...
  private registerTaskExecutors(): void {
    this.taskExecutors.set(
      ...
      new ExplanationExecutor(this.aiEngine)
    );
    this.taskExecutors.set(
      TaskType.CODE_MODIFICATION,
      new CodeModificationExecutor(this.aiEngine)
    );
    this.taskExecutors.set(
      TaskType.TEST_GENERATION,
      new TestGenerationExecutor(this.aiEngine)
    );
    this.taskExecutors.set(
      TaskType.DOCUMENTATION,
      new DocumentationExecutor(this.aiEngine)
    );
    this.taskExecutors.set(
      TaskType.REFACTORING,
      new RefactoringExecutor(this.aiEngine, this.codebaseIndexer)
    );
    this.taskExecutors.set(
      TaskType.IMPROVEMENT_NOTE,
      new ImprovementNoteExecutor(this.aiEngine, {
        setStatusBar: (text: string, tooltip?: string) =>
    ...
    this.taskExecutors.set(
      TaskType.TESTING,
      new TestingExecutor(this.aiEngine, {
        setStatusBar: (text: string, tooltip?: string) =>
          this.setStatusBarMessage(text, tooltip),
        showLoading: (message?: string) => this.showLoading(message),
        showReady: (message?: string) => this.showReady(message),
    ...
    this.taskExecutors.set(
      TaskType.DEBUGGING,
      new DebuggingExecutor(this.aiEngine, {
        setStatusBar: (text: string, tooltip?: string) =>
          this.setStatusBarMessage(text, tooltip),
        showLoading: (message?: string) => this.showLoading(message),
        showReady: (message?: string) => this.showReady(message),
    ...
    this.taskExecutors.set(
      ...
      new OptimizationExecutor(this.aiEngine, {
        setStatusBar: (text: string, tooltip?: string) =>
          this.setStatusBarMessage(text, tooltip),
        showLoading: (message?: string) => this.showLoading(message),
        showReady: (message?: string) => this.showReady(message),
    ...
    this.taskExecutors.set(
      TaskType.SECURITY,
      new SecurityExecutor(this.aiEngine, {
        setStatusBar: (text: string, tooltip?: string) =>
          this.setStatusBarMessage(text, tooltip),
        showLoading: (message?: string) => this.showLoading(message),
        showReady: (message?: string) => this.showReady(message),
    ...
    this.taskExecutors.set(
      TaskType.REVIEW,
      new ReviewExecutor(this.aiEngine, {
        setStatusBar: (text: string, tooltip?: string) =>
          this.setStatusBarMessage(text, tooltip),
        showLoading: (message?: string) => this.showLoading(message),
        showReady: (message?: string) => this.showReady(message),
  ...
  private registerCommands(): void {
      ...
      () => {
        if (this.aiAssistantPanel) {
          ...
          vscode.window.showInformationMessage(
            "Open Smile AI assistant first to attach files."
          );
        }
      }
    );
    ...
    const attachFolderCommand = vscode.commands.registerCommand(
      "smile-ai.attachFolder",
      () => {
        if (this.aiAssistantPanel) {
          this.attachFolder();
        } else {
          vscode.window.showInformationMessage(
            "Open Smile AI assistant first to attach folders."
          );
        }
      }
    );
      ...
      async (improvement) => {
        if (this.aiAssistantPanel) {
          ...
          await vscode.window.showInformationMessage(
            "Improvement registered: " + improvement
          );
        }
      }
    );
