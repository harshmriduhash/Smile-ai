...
export class RAGService {
   ...
   */
  public async enhanceQueryWithContext(
    query: string
  ): Promise<{ enhancedQuery: string; relevantContext: string }> {
    if (!this.enabled) {
  ...
  ): Array<{ file: IndexedFile; score: number }> {
   ...
   */
  private formatRelevantContext(
    relevantDocuments: Array<{ file: IndexedFile; score: number }>
  ): string {
    ...
    for (const { file, score } of relevantDocuments) {
      ...
      context += `File: ${relativePath} (relevance: ${(score * 100).toFixed(
        1
      )}%)\n`;
      context += "```\n";
      context += content;
