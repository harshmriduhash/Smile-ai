...
export class CodebaseIndexer {
  ...
  public async indexWorkspace(
    ...
    try {
      ...
      console.log(
        `Found ${workspaceFolders.length} workspace folders to index`
      );
    ...
    try {
      ...
      for (const file of files) {
        try {
          if (progressCallback) {
            progressCallback(
              `Indexing ${vscode.workspace.asRelativePath(file)} (${
                indexedCount + 1
              }/${files.length})`
            );
          }
      ...
      console.log(
        `Folder indexing completed: ${folderPath}. Indexed ${indexedCount}/${files.length} files`
      );
    } catch (error) {
