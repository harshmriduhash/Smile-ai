...
export class CodebaseIndex {
  ...
  public async buildIndex(
    ...
    if (this.generateEmbeddingsEnabled && !this.aiEngine) {
      ...
      console.error(
        "[CodebaseIndex] Cannot generate embeddings because AI Engine has not been set via setAIEngine()."
      );
      vscode.window.showErrorMessage(
        "Smile AI: Cannot generate embeddings - AI Engine not ready."
      );
    ...
    try {
      ...
      if (parsedCommandLine.errors.length > 0) {
        ...
        console.warn(
          ...
          parsedCommandLine.errors.map((d) =>
      ...
      progress?.report({
        message: `Creating program with ${rootFileNames.length} files...`,
        increment: 10,
      });
      console.log(
        `Creating program with ${rootFileNames.length} root files...`
      );
      ...
      console.log(
        `Program created with ${allSourceFiles.length} total source files.`
      );
      // -----------------------------

      progress?.report({
        message: `Analyzing ${allSourceFiles.length} files...`,
        increment: 10,
      });
      ...
      console.log(
        `Codebase indexing finished in ${
          (endTime - startTime) / 1000
        } seconds. Analyzed ${processedCount} files.`
      );
      vscode.window.showInformationMessage(
        `Codebase indexing finished (${processedCount} files).`
      );
    } catch (error) {
  ...
  private async parseFileWithProgram(
      ...
      const visitNodeAsync = async (node: ts.Node): Promise<void> => {
          ...
          try {
            ...
            if (symbol) {
              ...
              if (declarations && declarations.length > 0) {
                ...
                const definitionFilePath = vscode.workspace.asRelativePath(
                ...
                const definitionFileData =
                  this.indexData.get(definitionFilePath);
                let definitionSymbolInfo: SymbolInfo | undefined;
                if (definitionFileData?.symbols) {
                  const defStartPos =
                    declarationFile.getLineAndCharacterOfPosition(
                      declarationStart
                    );
                  definitionSymbolInfo = definitionFileData.symbols.find(
                    (def) =>
                ...
                if (definitionSymbolInfo) {
                  ...
                  const callEndPos = sourceFile.getLineAndCharacterOfPosition(
                    expression.getEnd()
                  );
                  const range = new vscode.Range(
                  ...
                  const reference = new vscode.Location(
                    vscode.Uri.file(sourceFile.fileName),
                    range
                  );
            ...
            console.error(
              `TypeChecker error processing call at ${filePath}:${
        ...
        if (symbolInfo && symbolName) {
          try {
            ...
            if (this.generateEmbeddingsEnabled && this.aiEngine) {
              try {
                const symbolCode = node
                  .getSourceFile()
                  .text.substring(node.getStart(), node.getEnd());
                const maxEmbedLength = 1000;
                const truncatedCode =
                  symbolCode.length > maxEmbedLength
                    ? symbolCode.substring(0, maxEmbedLength)
                    : symbolCode;
                definition.embedding = await this.aiEngine.generateEmbeddings(
                  truncatedCode
                );
              } catch (embedError) {
                console.warn(
                  `[CodebaseIndex] Failed to generate embedding for symbol '${definition.name}' in ${filePath}:`,
                  embedError
                );
              }
            }
            ...
            console.error(
              `Error getting position for symbol ${symbolName} in ${filePath}:`,
              posError
            );
          }
        }
  ...
  public findSimilarSymbols(
    ...
    if (!this.generateEmbeddingsEnabled) {
      console.warn(
        "[CodebaseIndex] Cannot find similar symbols: Embeddings generation is disabled."
      );
      // Optionally throw an error or return empty
      vscode.window.showWarningMessage(
        "Cannot perform semantic search: Embedding generation is disabled in Smile AI settings."
      );
      return [];
    }
    ...
    console.log(
      `[CodebaseIndex] Searching for symbols similar to query vector (${queryEmbedding.length} dimensions)...`
    );
    const startTime = Date.now();
    ...
    for (const fileData of this.indexData.values()) {
      if (fileData.symbols) {
        for (const symbol of fileData.symbols) {
          if (
            symbol.embedding &&
            symbol.embedding.length === queryEmbedding.length
          ) {
            try {
              ...
              console.error(
                `Error calculating similarity for symbol ${symbol.name}:`,
                error
              );
    ...
    const endTime = Date.now();
    console.log(
      `[CodebaseIndex] Semantic search completed in ${
        endTime - startTime
      }ms. Found ${results.length} potential matches.`
    );
  ...
  private convertSyntaxKindToSymbolKind(
    kind: ts.SyntaxKind
  ): vscode.SymbolKind {
    switch (kind) {
