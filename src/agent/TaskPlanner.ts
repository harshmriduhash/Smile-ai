...
export class TaskPlanner {
  ...
  public async planTask(description: string): Promise<Task> {
    ...
    const taskAnalysis = await this.analyzeTaskWithAI(
      description,
      fileContext,
      codeAnalysis
    );
  ...
  public async breakdownTask(task: Task): Promise<Task[]> {
    ...
    const subtaskAnalysis = await this.getSubtasksFromAI(
      task,
      fileContext,
      codeAnalysis
    );
  ...
  private async getSubtasksFromAI(
    ...
    const prompt = this.buildSubtaskAnalysisPrompt(
      task,
      fileContext,
      codeAnalysis
    );
...
1. Determine the most appropriate task type from: ${Object.values(
      TaskType
    ).join(", ")}
...
`;
  ...
  private getAffectedClasses(codeAnalysis: CodeAnalysis): string {
    return (
      codeAnalysis.structure.classes.map((c) => c.name).join(", ") || "None"
    );
  }

  private getAffectedFunctions(codeAnalysis: CodeAnalysis): string {
    return (
      codeAnalysis.structure.functions.map((f) => f.name).join(", ") || "None"
    );
  }
}
