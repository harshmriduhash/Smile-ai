...
export class RefactoringExecutor extends BaseExecutor implements TaskExecutor {
  ...
  public async execute(task: Task): Promise<TaskResult> {
    try {
      ...
      const approved = await this.showRefactoringPreview(
        changes,
        refactoringPlan
      );
  ...
  private async createRefactoringPlan(task: Task): Promise<RefactoringPlan> {
    ...
    if (selection && !selection.isEmpty) {
      ...
      const symbol = await this.codebaseIndexer.findSymbolAtPosition(
        document.uri,
        midPointPos
      );
      targetSymbol = symbol
      ...
      if (
        targetSymbol &&
        targetSymbol.startLine <= selection.start.line + 1 &&
        targetSymbol.endLine >= selection.end.line + 1 &&
        (targetSymbol.startLine !== selection.start.line + 1 ||
          targetSymbol.startChar <= selection.start.character) &&
        (targetSymbol.endLine !== selection.end.line + 1 ||
          targetSymbol.endChar >= selection.end.character)
      ) {
        console.log(
          `Refactoring symbol containing selection: ${targetSymbol.name}`
        );
        const symbolRange = new vscode.Range(
          targetSymbol.startLine - 1,
          targetSymbol.startChar,
          targetSymbol.endLine - 1,
          targetSymbol.endChar
        );
        codeToRefactor = document.getText(symbolRange);
        promptContext = `the ${getSymbolKindName(targetSymbol.kind)} '${
          targetSymbol.name
        }' in file ${filePath}`;
        ...
        promptContext = `the selected code snippet in ${filePath} (lines ${
      ...
      if (targetSymbol) {
        ...
        console.error(
          "Cannot determine target for refactoring without selection or symbol at cursor."
        );
        throw new Error(
          "Please select the code to refactor or place the cursor inside a specific symbol."
        );
      }
    }
    ...
    if (targetSymbol) {
      references = await this.codebaseIndexer.findReferences(
        document.uri,
        editor.selection.active
      );
      console.log(
        `Found ${references.length} references for symbol ${targetSymbol.name}`
      );
    }
    // ---------------------
    ...
    if (suggestions.length > 0) {
      console.log(
        `[RefactoringExecutor] Found ${suggestions.length} improvement suggestions in plan.`
      );
      let noteContext: ImprovementNoteContext | undefined = undefined;
      if (editor) {
        // Use editor from createRefactoringPlan scope
        noteContext = { filePath: filePath };
        if (targetSymbol) {
          // Use targetSymbol from createRefactoringPlan scope
      ...
      for (const suggestion of suggestions) {
        try {
          ...
          vscode.window.showInformationMessage(
            `Improvement suggestion noted: ${suggestion.substring(0, 30)}...`
          );
          console.log(
            `[RefactoringExecutor] Automatically noted improvement from plan: ${suggestion.substring(
              0,
              50
            )}...`
          );
          ...
          console.error(
            "Error automatically noting improvement from plan:",
            noteError
          );
        }
      }
  ...
  private buildRefactoringPlanPrompt(
    ...
    if (references.length > 0) {
      ...
      const refsByFile = references.reduce((acc, ref) => {
        (acc[ref.uri.fsPath] = acc[ref.uri.fsPath] || []).push(
      ...
      for (const [filePath, lines] of Object.entries(refsByFile)) {
        referencesSection += `- ${filePath} (Lines: ${(lines as number[])
...
Provide your refactoring plan in this JSON format (locations relative to snippet start):
{
  ...
  private async generateRefactoringChanges(
    plan: RefactoringPlan
  ): Promise<RefactoringChanges> {
...
Generate the code changes in JSON format matching RefactoringChanges interface.`;
  ...
  private async showRefactoringPreview(
...
<body>
        ...
        <div class="section analysis">
            ...
            ${
              plan.analysis.codeSmells?.length > 0
                ? `
                <h3>Code Smells</h3>
                ${plan.analysis.codeSmells
                  .map(
                    (smell) => `
                    <div class="issue">
                        <span class="tag ${smell.severity}">${
                      smell.severity
                    }</span>
                        ...
                        ${
                          smell.location
                            ? `<div><small>Location: Lines ${smell.location.startLine}-${smell.location.endLine}</small></div>`
                            : ""
                        }
                    </div>
                `
            ...
            `
             ...
             ${
               plan.analysis.designIssues?.length > 0
                 ? `
                <h3>Design Issues</h3>
                ${plan.analysis.designIssues
                  .map(
                    (issue) => `
                    <div class="issue">
                        <h4>${this.escapeHtml(issue.type)}</h4>
                        <p>${this.escapeHtml(issue.description)}</p>
                        <p><strong>Suggested Pattern:</strong> ${this.escapeHtml(
                          issue.suggestedPattern
                        )}</p>
                        <p><small>Rationale: ${this.escapeHtml(
                          issue.rationale
                        )}</small></p>
                    </div>
                `
            ...
            ${
              plan.analysis.performanceIssues?.length > 0
                ...
                ${plan.analysis.performanceIssues
                  .map(
                    (issue) => `
                    <div class="issue">
                        <p>${this.escapeHtml(issue.description)}</p>
                        <p><strong>Impact:</strong> ${this.escapeHtml(
                          issue.impact
                        )}</p>
                        <p><strong>Suggestion:</strong> ${this.escapeHtml(
                          issue.suggestion
                        )}</p>
                    </div>
                `
                       ...
                       file.edits && file.edits.length > 0
                         ...
                         <div class="edit-detail">
                             <div class="edit-info">
                                 ...
                                 <span class="edit-type">${this.escapeHtml(
                                   edit.type || "MODIFY"
                                 )}</span>
                                 <!-- Display range -->
                                 <span class="edit-location">Lines ${
                                   edit.range.startLine
                                 }-${edit.range.endLine}</span>
                             </div>
                             <!-- Display description if available -->
                             ${
                               edit.description
                                 ? `<p class="edit-description">${this.escapeHtml(
                                     edit.description
                                   )}</p>`
                                 : ""
                             }
                          ...
                          .map(
                            ...
                            <li class="risk-item">
                              ...
                              risk.description
                                ...
                                <br>
                                <small>Mitigation: ${this.escapeHtml(
                                  risk.mitigation
                                )}</small>
                            </li>
                        `
            ...
            <div class="testing">
                ...
                ${
                  plan.testingStrategy?.impactedTests?.length > 0
                    ? `
                    <ul>${plan.testingStrategy.impactedTests
                      .map((test) => `<li>${this.escapeHtml(test)}</li>`)
                      .join("")}</ul>
                `
                    : "<p>None specified.</p>"
                }
                ...
                ${
                  plan.testingStrategy?.newTestsNeeded?.length > 0
                    ? `
                     <ul>${plan.testingStrategy.newTestsNeeded
                       .map((test) => `<li>${this.escapeHtml(test)}</li>`)
                       .join("")}</ul>
                 `
                    : "<p>None specified.</p>"
                ...
                <p><strong>Regression Risks:</strong></p>
                 ${
                   plan.testingStrategy?.regressionRisks?.length > 0
                     ? `
                     <ul>${plan.testingStrategy.regressionRisks
                       .map((risk) => `<li>${this.escapeHtml(risk)}</li>`)
                       .join("")}</ul>
                 `
                     : "<p>None specified.</p>"
                 }
  ...
  private async applyRefactoring(changes: RefactoringChanges): Promise<void> {
    console.warn(
      "[RefactoringExecutor] applyRefactoring currently only supports single-file changes based on AI plan structure. Multi-file reference updates are NOT implemented."
    );
  ...
  private adjustPlanLocations(
    ...
    if (plan.analysis?.codeSmells) {
      plan.analysis.codeSmells.forEach((smell) => {
        if (smell.location) {
          smell.location.startLine =
            Math.max(1, smell.location.startLine) + baseLineOffset;
          smell.location.endLine =
            Math.max(1, smell.location.endLine) + baseLineOffset;
        }
      });
    }
    if (plan.changes) {
      plan.changes.forEach((change: any) => {
        if (change.steps) {
          change.steps.forEach((step: any) => {
            if (step.target) {
              step.target.startLine =
                Math.max(1, step.target.startLine) + baseLineOffset;
              step.target.endLine =
                Math.max(1, step.target.endLine) + baseLineOffset;
            }
          });
        }
      });
  ...
  private extractSuggestionsFromPlan(plan: RefactoringPlan): string[] {
    ...
    plan.analysis?.codeSmells?.forEach((smell) =>
      checkContent(smell.description)
    );
    plan.analysis?.designIssues?.forEach((issue) =>
